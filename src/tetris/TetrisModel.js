...
class Figure {
  ...
  doRotate() {
    ...
    return (
      this._move(dim, this._cx, this._cy) ||
  ...
  _canPlace(dim, x, y) {
    return !dim.find((p) => {
      ...
      if (ix < 0 || ix >= this.model.maxCols || iy < 0 || iy >= this.model.maxRows) return true;
  ...
  _move(dim, x, y) {
    if (!this._canPlace(dim, x, y)) return false;
  ...
  _fill(state) {
    this._dim.forEach((p) => {
      ...
      if (y < this.model.maxRows) this.board[y][this._cx + p.x] = state;
    });
  }
...
export default class TetrisModel {
  ...
  init() {
    ...
    this._board = [];
    for (let y = 0; y < this.maxRows; y++) this._newRow(y);
  }
  ...
  start() {
    ...
    if (!this._figure) this._figure = new Figure(this);
  ...
  down() {
    ...
    if (this._figure.doMove(0, +1)) return true;
  ...
  moveLt() {
    return this.active && this._figure.doMove(-1, 0);
  }
  ...
  rotate() {
    return this.active && this._figure.doRotate();
  }
  ...
  _checkRows() {
    const scroll = (ridx) => {
      for (let y = ridx; y > 0; y--) this.board[y] = this.board[y - 1];
      this._newRow(0);
    };
